...
async function loadActiveGroupTabs(): Promise<{
  groupTitle: string;
  tabs: Tab[];
}> {
...
export default function SwitchTab() {
  const { data, isLoading, error, revalidate } =
    usePromise(loadActiveGroupTabs);
  ...
  return (
    <List
      ...
      searchBarPlaceholder={
        groupTitle ? `Search tabs in ${groupTitle}...` : "Search tabs..."
      }
    >
      {tabs.length === 0 && !isLoading ? (
        <List.EmptyView
          icon={Icon.Globe}
          title={
            groupTitle ? `No tabs in ${groupTitle}` : "No active tab group"
          }
          description={
            groupTitle
                  ...
                  <Action
                    ...
                    onAction={async () => {
                      try {
                        ...
                        try {
                          await raiseTabWindow(tab.id);
                        } catch (e) {
                          console.warn("raiseTabWindow failed", e);
                        }
